...
export const pendingPhotoUploads = sqliteTable(
  "pending_photo_uploads",
  {
    ...
    lens: text("lens"),
    camera: text("camera"),
    status: text("status", { enum: ["pending", "finalized"] })
      .notNull()
      .default("pending"),
    created_at: text("created_at")
      .notNull()
  ...
  (table) => [
    uniqueIndex("pending_photo_uploads_upload_id_unique").on(table.upload_id),
    uniqueIndex("pending_photo_uploads_idempotency_unique").on(table.idempotency_key),
    uniqueIndex("pending_photo_uploads_photo_id_unique").on(table.photo_id),
    index("pending_photo_uploads_session_idx").on(table.session_id),
    index("pending_photo_uploads_status_idx").on(table.status),
  ],
);
...
export const pendingPhotoUploadsRelations = relations(pendingPhotoUploads, ({ one }) => ({
  session: one(sessions, {
    fields: [pendingPhotoUploads.session_id],
    references: [sessions.id],
  }),
  photo: one(photos, {
