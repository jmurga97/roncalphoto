...
function ThumbnailComponent({ photo, index, isActive, onClick }: ThumbnailProps): ReactNode {
  ...
  return (
    ...
    >
      <img src={photo.miniature} alt="" loading="lazy" className="h-full w-full object-cover" />
    </button>
  );
}
...
export const Thumbnail = memo(ThumbnailComponent, (prev, next) => {
  return (
    prev.photo.id === next.photo.id && prev.index === next.index && prev.isActive === next.isActive
  );
});
