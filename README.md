...
# D1
bun run --filter=@roncal/api db:generate
bun run --filter=@roncal/api db:migrations:list
bun run --filter=@roncal/api db:migrate
```
...
- La API llama al worker remoto de email exclusivamente mediante el service binding `EMAIL_WORKER`.
...
- Las migraciones se generan explicitamente con `db:generate`; `dev` no modifica el esquema y `db:migrate` aplica solo migraciones versionadas sobre la D1 remota.
- No existen seeds ni copias locales. Los usuarios, sesiones de auth y datos de portfolio remotos son la unica fuente de verdad.
