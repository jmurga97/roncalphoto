...
function scrollToThumbnail(track: HTMLDivElement | null, index: number): void {
  ...
  const thumbnail = track.querySelector(`[data-thumbnail-index="${index}"]`) as HTMLElement | null;

  if (!thumbnail) return;

  const scrollPosition = thumbnail.offsetLeft - track.clientWidth / 2 + thumbnail.clientWidth / 2;
...
function CarouselTrack({ trackRef, onThumbnailClick }: CarouselTrackProps): ReactNode {
 ...
 */
export function PhotoCarousel({ mainContainerRef, desktopRef }: PhotoCarouselProps): ReactNode {
  ...
  return (
    <nav className="flex-[0.15] bg-neutral-900/50 overflow-hidden" aria-label="Miniaturas de fotos">
      <div className="h-full">
        <CarouselTrack trackRef={desktopRef} onThumbnailClick={handleThumbnailClick} />
      </div>
    </nav>
  );
}
